export default async function handler(req, res) {
  ...
  try {
    ...
    const prompt = `
You are helping estimate nutrition macros from a meal photo.
Return ONLY valid JSON with these exact keys:
mealName, calories, protein, carbs, fat, notes.

Rules:
- Estimate visible food only.
- Use grams for protein/carbs/fat.
- Calories must be a number.
- If uncertain, explain uncertainty in notes.
- Keep notes short.
