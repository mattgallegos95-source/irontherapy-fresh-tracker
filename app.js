const workoutTemplates = {
  "Back & Traps": [
    ...
    ["Barbell or Dumbbell Shrugs", "https://www.youtube.com/results?search_query=dumbbell+shrugs+proper+form", "4 sets / 10-15 reps"],
  ...
  "Arms": [
    ["Barbell Curl", "https://www.youtube.com/results?search_query=barbell+curl+proper+form", "4 sets / 8-12 reps"],
    ["Incline Dumbbell Curl", "https://www.youtube.com/results?search_query=incline+dumbbell+curl+proper+form", "3 sets / 10-12 reps"],
    ["Close-Grip Bench Press", "https://www.youtube.com/results?search_query=close+grip+bench+press+proper+form", "4 sets / 8-10 reps"],
    ["Skull Crushers", "https://www.youtube.com/results?search_query=skull+crusher+proper+form", "3 sets / 10-12 reps"],
    ["Rope Pushdowns", "https://www.youtube.com/results?search_query=rope+pushdown+proper+form", "3 sets / 12-15 reps"]
  ],
  "Push Day": [
    ["Bench Press", "https://www.youtube.com/results?search_query=bench+press+proper+form", "4 sets / 6-10 reps"],
    ["Incline Dumbbell Press", "https://www.youtube.com/results?search_query=incline+dumbbell+press+proper+form", "4 sets / 8-10 reps"],
...
let state = JSON.parse(localStorage.getItem("ironTheropyFreshBuildV1") || "{}");
let today = new Date();
...
function save(){ localStorage.setItem("ironTheropyFreshBuildV1", JSON.stringify(state)); renderCalendar(); updateSummary(); }
...
function renderCalendar(){
  ...
  for(let i=firstDay-1;i>=0;i--) addDay(prevDays-i,true);
  for(let d=1; d<=daysInMonth; d++) addDay(d,false);
  const count = cal.children.length;
  for(let d=1; d<=42-count; d++) addDay(d,true);

  function addDay(num, muted){
    const div=document.createElement("div");
    div.className="day"+(muted?" muted":"");
    const key = muted ? "" : dateKey(currentYear,currentMonth,num);
    if(key===selectedDate) div.classList.add("selected");
    div.innerHTML = `<div class="date-num">${num}</div><div class="badges"><span class="badge fit"></span><span class="badge nut"></span><span class="badge task"></span></div>`;
    if(!muted){
...
function renderFitness(){
  ...
  absList.style.display=data.absEnabled ? "block" : "none";
  if(data.absEnabled) renderExerciseList(absList, data.abs, false);
  else absList.innerHTML="";
...
function renderNutrition(){
  ...
  data.meals.forEach((meal,idx)=>{
    ...
    tpl.querySelector(".estimate-btn").onclick=()=>alert("Auto macro estimates need a hosted AI vision API connection. For now, upload the photo and enter/edit the macros manually.");
...
document.getElementById("exportBtn").onclick=()=>{const blob=new Blob([JSON.stringify(state,null,2)],{type:"application/json"}); const a=document.createElement("a"); a.href=URL.createObjectURL(blob); a.download="irontherapy-fresh-backup.json"; a.click();}
