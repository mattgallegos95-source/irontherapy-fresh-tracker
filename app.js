...
function renderNutrition(){
  ...
  data.meals.forEach((meal,idx)=>{
    ...
    tpl.querySelector(".estimate-btn").onclick = async () => {
      ...
      if (!meal.photo) {
        setStatus("Upload a meal photo first.");
        alert("Upload a meal photo first, then tap Estimate Macros.");
        return;
      }
      ...
      try {
        ...
        try {
          ...
          throw new Error("The server did not return JSON. Check Vercel function logs.");
        }

        if (!response.ok) {
          throw new Error(result.error || "Meal analysis failed.");
        }
        ...
        meal.notes = result.notes || "AI estimate. Review serving size for accuracy.";

        save();
        renderNutrition();
        alert("Meal macros estimated. Review and adjust serving size if needed.");
      } catch (err) {
        const message = "AI meal analysis failed: " + err.message;
        setStatus(message);
        alert(message);
      } finally {
